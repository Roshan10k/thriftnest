...
export function LoginPage() {
  ...
  // Surface a friendly message if the Google OAuth round-trip bounced back with
  // an error (the backend redirects here with ?oauth=failed|error|unconfigured).
  useEffect(() => {
    ...
    const message =
      reason === 'unconfigured'
        ...
        : 'Google login did not complete. Please try again or use email and password.';
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if ('mfaRequired' in payload && payload.mfaRequired) {
        setPendingUserId(payload.userId ?? null);
        setStep('mfa');
        return;
      }
        ...
        {step === 'credentials' && (
          <div className="bg-thrift-surface border border-thrift-border rounded-card p-6 shadow-card fade-in">
            ...
            <div className="space-y-4">
              ...
              <Button variant="outline" className="w-full" onClick={() => { window.location.href = googleLoginUrl; }} icon={
